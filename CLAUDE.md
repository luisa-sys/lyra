...
23. **The npm-audit gate is PROD-TREE scoped and lives in ONE place — and the technique for a transitive dep whose only patched version breaks an older consumer is a *scoped* override, never a blanket one**:
    ...
    **The override technique below is still correct, and is now needed far less often — but the lesson in it is the durable half: a gate under time pressure produces brittle pins.** 2026-07-25 (SEC-94): 36 HIGH traced to `postcss` (simple override → 8.5.23) and `brace-expansion ≤5.0.7` (GHSA-mh99-v99m-4gvg). brace-expansion's *only* patched release — **5.0.8 — is a breaking named export** (`require('brace-expansion')` now returns `{expand,…}`, not a callable), so a blanket `"brace-expansion":"5.0.8"` override clears the audit but **crashes `npm run lint`** (`TypeError: expand is not a function`) because eslint's old `minimatch@3` calls it as a function. The tree has two generations — old-gen minimatch@3 + callable brace-expansion (eslint core+plugins, glob@7 under rimraf/test-exclude) and new-gen minimatch@9/10 + named brace-expansion (jest/sentry/typescript-eslint) — and **no bridge version exists** (minimatch@4–8 are callable but use the old import; minimatch@9+ are non-callable → break glob@7). **Fix without an eslint 9→10 major bump:** pin `"brace-expansion":"5.0.8"` tree-wide AND give *only eslint's own packages* a modern minimatch via nested overrides — `"@eslint/config-array":{"minimatch":"^10.2.5"}`, `"@eslint/eslintrc":{"minimatch":"^10.2.5"}`, `"eslint":{"minimatch":"^10.2.5"}` (eslint uses `new Minimatch()`, which is named-export-compatible); glob@7 + the eslint plugins keep minimatch@3 harmlessly (they don't brace-expand during lint/build/coverage). **Always verify a security override empirically before shipping** — on a clean `npm ci` run `npm run lint && npm run type-check && npm run build && npx jest --coverage`, and diff the lint output against the base branch (a safe override leaves it byte-identical: same files, same warnings). The npm `fixAvailable` field lies here (it proposes destructive major *downgrades*); ignore it and reason from the advisory's vulnerable range. Full write-up: SEC-94.
    ...
    ⚠️ **An override has no expiry, and that gap is what let a fix become the vulnerability (SEC-170, 2026-09-02).** `security/npm-audit-waivers.json` entries carry a dated `expires`/`review_by` that fails the build (or notifies) on lapse — CTL-025 enforces the hygiene. An audit-forced *override* in `package.json` carries none of that: it is a bare version pin with no ticket, no owner, no expiry field, and nothing in `scripts/check-npm-audit-gate.py` or anywhere else tracks whether a pinned version is still the patched one (confirmed by grep — zero references to `overrides` in that script). `fast-uri@^3.0.0` was pinned to `3.1.5` as the SEC-94-era fix; when the vulnerable range later grew to include `3.1.5`, the pin silently held the tree at a now-vulnerable version with no red build until the *audit* gate caught it from the outside. The two obvious remedies are both no-ops against an override — verified, not assumed: `npm update fast-uri --package-lock-only` changes nothing, and deleting the lock entry to force a re-resolve still returns `3.1.5`, because the override wins either way. Only editing the override value (`3.1.7`) freed the tree. Whether to add expiry/review tracking to overrides the way waivers have it is open — SEC-170 prevention option (a), Luisa's call.
...
26. **`REVOKE ... FROM PUBLIC` does NOT remove `anon`/`authenticated` on Supabase — this is why revoking EXECUTE has regressed nine times**:
    ...
    **The rule:** every migration revokes from **all three** roles explicitly.
    ...
    ✅ **THE DEFAULT IS NOW INVERTED ON DEV AND STAGING (SEC-103, 2026-08-20).** `supabase/migrations/20260820090000_sec103_invert_default_function_grant.sql` revokes the `anon`/`authenticated` EXECUTE default **and** the built-in PUBLIC one, so a forgotten grant now yields a 403 instead of a data leak. Applied to `dev-lyra` and `stage-lyra`; **production is Luisa's and has NOT been done** — until it is, prod still ships every new function anon-callable, and Supabase forces the change platform-wide on **2026-10-30** regardless.

    Measured, not assumed. A negative control ran on dev *before* the migration: a throwaway SECURITY DEFINER function was born `{=X/postgres,postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}` — both vectors at once, the leading `=X/` being the PUBLIC grant. The identical probe after: `{postgres=X/postgres,service_role=X/postgres}`, `has_function_privilege` false for both roles. Same result on staging.

    ⚠️ **Three things about it that are easy to get wrong:**

    - **The two revoke statements must NOT match, and the natural instinct is to make them match.** The per-schema one names `in schema public`; the PUBLIC one **must not**, because that default is global — qualify it and it succeeds, writes a `pg_default_acl` row, and does nothing (supabase/supabase#43884). Pinned by `tests/unit/sec103-invert-default-function-grant.test.ts`, mutation-proven: adding `in schema public` to the global statement reddens 3.
    - **`ALTER DEFAULT PRIVILEGES` changes nothing that already exists.** No current ACL moved, and `CREATE OR REPLACE` still leaves an ACL untouched, so nothing that worked before the migration stopped working. What it locks down is the *next* function — including the signature-change overload that is this gotcha's second vector.
    - **STATED GAP — one grantor is unreachable.** `pg_default_acl` carries **two** grantors for `public`/`f`: `postgres` and `supabase_admin`. Migrations run as `postgres`, which is not a member of `supabase_admin`; the attempt was made and refused with `42501: permission denied to change default privileges`. Bounded: all 42 functions in `public` on dev are owned by `postgres`, so the unreached default can only bite for a function the *platform* creates in `public`, which no Lyra migration does. Closing it needs a superuser — Luisa's, tracked on SEC-103, not silently dropped.

    **And note what the enumeration cost.** After inversion the list of explicit grants is load-bearing, and a dead call site reads exactly like a live one to grep. A naive sweep of `.rpc(` + cookie-client yields **four** functions; the fourth is `get_metrics_for_window`, whose two `createClient()` callers (`getAnomalyWindow`, `getMetricsForWindow`) have **no callers of their own** — the live path is `getAnomalyWindowAdmin` via service-role, exactly because BUGS-71 found the authenticated read failing. Granting it would have re-opened F-14 / SEC-07 while looking like completeness.
    ...
    **Guards (SEC-101):** `scripts/check-migration-privileges.py` (in `pr-checks.yml`) fails any PR whose migration breaks these rules, ratcheted against `supabase/migration-privileges-baseline.json` — 21 pre-existing violations are grandfathered and that list may only shrink. Because migrations are applied with the Supabase MCP `apply_migration` tool and can reach a database **without passing through a PR** (exactly how SEC-42 happened), the live backstop is `scripts/check-db-invariants.py` + `.github/workflows/db-invariants.yml`, which asserts seven invariants against dev, staging and production daily. ⚠️ **That daily assertion has not actually produced a result since 2026-08-14.** Every scheduled run since — 31 consecutive as of 2026-09-13, and still failing as of this check — dies in the `Committed schema snapshots match their databases (CTL-048, daily)` job with `The admin schema contract could not be READ or compared`: UNVERIFIED, not a drift finding, so the backstop has given no real answer for a month, not "no problems found." Filed as **SEC-164** (2026-08-18, status To Do, still zero comments as of 2026-09-21 — nobody has picked it up; now 38 consecutive scheduled failures as of run #60, 2026-09-20). ⚠️ **Corrected 2026-09-21 (KAN-359 second pass):** the prior text here said CTL-072 (`scripts/check-workflow-run-freshness.py`, landed on `develop` 2026-09-13) "will report it STALE once its first `required-checks.yml` run fires" — implying that was imminent. **It is not imminent; it is structurally blocked.** `required-checks.yml` is `schedule`-triggered, and a `schedule` trigger always runs the workflow file version committed on the repository's **default branch** — `main`, not `develop` (the concrete case of gotcha #1's promotion chicken-and-egg). `main` was last promoted 2026-08-17 and is 150 commits behind `develop`, so `main`'s copy of `required-checks.yml` still has only the original `drift` job. Confirmed directly: every scheduled `required-checks.yml` run since CTL-072 merged (#28–#35, 2026-09-13 through 2026-09-20 — 8 fires) ran exactly one job, `Branch protection matches the committed expectation`, with no `freshness` job in any of them. So SEC-164's silence has not been broken, and cannot be, until `required-checks.yml` itself rides the promote chain onto `main` — the control built to catch a silent control is, itself, currently silent, for the most ordinary possible reason. Allow-list a migration with `-- db-privileges-ok: <reason>` plus a SEC key.
...
31. **A file move silently DELETES its `SRC` manifest key — and `SRC.gone` is `undefined`, which makes negative assertions vacuously true**: `tests/support/source-paths.ts` exists so tests reference `SRC.profileActions` instead of hard-coding a path, and `scripts/gen-test-paths.mjs` builds it by harvesting path literals from `tests/**`, keeping only the ones that still resolve on disk.
    ...
    **The rule:** when you move a file any test reaches via `SRC`, either update the value in the manifest in the same commit (the loop then re-sustains it), or add it to **`tests/support/source-path-seeds.ts`**. Seeding is durable — it survives a regeneration performed from a stale checkout, which is how the key was lost the first time. `tests/support/**` is excluded from the F4 raw-literal ratchet precisely because "the manifest is where the literals are supposed to live", so seeding is *aligned* with that ratchet, not a way around it.
    ...
    ⚠️ **The quieter half is a key that still resolves and now means something ELSE (KAN-475).** Because a name is derived by widening leftwards *until unique*, it depends on which other paths are present — so a move can **repoint** a key belonging to a file that did not move. Two files ended in `pipeline.ts`; moving only the recommender one left `SRC.pipeline` pointing at `src/modules/access/pipeline.ts`, and a test went on reading a file it was never written for. Nothing about that is detectable from the test's shape: no `undefined`, no throw, no dangling path, and the integrity test above is **satisfied**, because every key still points at a tracked file. It failed only because that one assertion happened to look for a specific symbol — phrased as `toContain('export')`, or any `not.toContain(...)`, it would have reported green forever. Re-measured on the real tree while building the fix: the case `SRC.pipeline is defined and points at a tracked file` **passed throughout**. That matters because CTL-013's signup-surface fixtures, CTL-028's suspension coverage and the KAN-411 ownership test all reach their subject through `SRC` — a repointed key there makes a **security** guard assert against the wrong file while reporting green.

    **Guard (CTL-070):** `scripts/check-source-path-identity.py` (in `pr-checks.yml`, with `--self-test`) pins key → path in `tests/support/source-path-identity.json`. Two-way: a **repointed** key fails, and an added/removed key fails as **staleness**. The two are deliberately not interchangeable — a plain `--write` clears staleness but **refuses** a repoint, which can only be cleared by naming it: `--write --accept-repoint=<key>`. There is no annotation escape hatch, because the point is that re-declaration is an explicit act by whoever made the move. ⚠️ **`npm run gen:test-paths` must never write this lock** — a lock the generator maintains is the manifest again, and the control would agree with itself forever; a test asserts the generator leaves it untouched.
