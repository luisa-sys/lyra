...
- Daily 06:20 — Workflow-run freshness (`required-checks.yml`, second job, SEC-106 / CTL-072): asserts every workflow in `.github/workflow-freshness.json` has a run concluding **`success`** inside its window (`db-invariants.yml` 36h, `health-check.yml` 36h, `security-audit.yml` 192h). ⚠️ **Only `success` counts.** GitHub's successful-status set is `{success, skipped, neutral}`, so a check keyed on "a run exists" is satisfied by a job skipped because a `needs:` dependency failed — the `needs:`-skip trap, and most of the promote-reports-success cluster. It complements CTL-042 rather than duplicating it: that one asserts a workflow is not `disabled_manually`, this asserts it actually **ran**, and a workflow can be enabled and still never fire. Needs only `actions:read`, so unlike the drift job it reports a real measured answer today. Deliberately **not** `needs:`-chained to that job — chaining freshness behind a job expected to exit 2 would make it report SKIPPED daily, which is the trap it exists to detect. See `docs/RUNBOOK.md` → "Watched scheduled workflows have actually run".
