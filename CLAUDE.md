...
- Current test floor: **2659 tests** (225 suites) in lyra (unit + scripts; E2E + integration not counted), **734 tests** (44 suites) in lyra-mcp-server. **Measured 2026-07-31** at `lyra` develop `5d98e79` via `npm run test:unit`, and `lyra-mcp-server` main `31c114b` via `npx tsc && npm test`. Re-measure and update this line whenever it drifts — a floor far below reality cannot detect a regression that deletes hundreds of tests. (Previously 2611/222 measured 2026-07-28 under KAN-435; before that, 2118/172 and 91/5, both years of work stale.)
...
⚠️ **One half of BUGS-19 remains, and it is the half that matters now.** There are still **two deploy paths**, and only one is auditable. The CF Workers Builds Git integration is demonstrably still live (it deployed on 2026-07-30 via the 20+-commit watch-path bypass). Until it is disabled in the Cloudflare dashboard, a failed repo-owned deploy can be silently papered over by a CF-side build, which is the same shape as any other control whose failure is masked. **Founder action: disable the `lyra-maintenance` Workers Builds Git integration**, leaving `lyra-maintenance-deploy.yml` as the single path.

**Prevention: none yet — the durable control IS removing the second path.** With one deploy path, a worker change that fails to deploy is a red run on `main` that nobody can miss; with two, the outcome depends on which fired. No grep or gate substitutes for that.
