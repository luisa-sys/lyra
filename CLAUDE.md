...
| **CTL-035 guard-path-drift** | any path pattern in 20 registered artefacts matches no **tracked** file. ⚠️ **One of the 20 is registered `external` and is never failed in either direction** — `check-extraction-dod.sh`'s `ROUTINE_COUPLED` mirrors an out-of-repo claude.ai routine prompt, so an absent path there is the expected state after a move, not drift (KAN-474). What registration buys is that the block cannot silently vanish (renaming or emptying it exits 2) and every entry is printed with its current tree state; it does NOT verify the entry matches what any prompt says — nothing can, which is why the `routine-prompts` attestation exists | `# guard-path-ok: <JIRA-KEY> <reason>` |
