...
Founder fix: add **Workers Scripts:Edit** + **Workers KV Storage:Edit** to that token (or mint a dedicated one and update the secret), then prove it with `gh workflow run lyra-maintenance-deploy.yml --ref main`. Because the worker source is unchanged, a successful run is a **no-op redeploy** — safe to use purely as a proof. **Do not treat BUGS-19 as resolved until that run is green** — and close it properly by then disabling the CF Workers Builds Git integration, so there is exactly one deploy path and it is the auditable one.
