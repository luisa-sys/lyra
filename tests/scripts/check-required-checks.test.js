/**
 * CTL-064 (SEC-106) — a required status check must actually be satisfiable.
 *
...
describe('check-required-checks.py (CTL-064)', () => {
  ...
  test('is registered in the control registry', () => {
    ...
    const entry = controls.find((c) => c.id === 'CTL-064');
    expect(entry).toBeDefined();
...
describe('CTL-064 — the finding it was built from is real', () => {
