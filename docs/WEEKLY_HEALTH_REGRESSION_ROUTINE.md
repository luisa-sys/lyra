...
| 2026-07-02 | Weekly Health+Regression (Claude Code, remote) | **Green, no product FAIL.** Health: `checklyra.com/api/health` `{ok:true,…production}`, `mcp.checklyra.com/health` `ok`; develop HEAD `3800836` CI (Deploy to Development) + CodeQL both green. lyra runner: lint/type-check/unit/scripts/build **PASS=5**; `integration` **UNVERIFIED** (no `tests/integration/` dir — layout gap, BUGS-51); `e2e` **REAL-GREEN this run — 17 passed / 5 skipped (chromium)**. The raw script still marks `e2e` UNVERIFIED because `npx playwright install` (rev 1223) is CDN-blocked, but I verified E2E by pointing Playwright at the pre-baked env chromium (rev 1194) — symlinked `chromium-1223`→`chromium-1194` **and** the expected `chrome-headless-shell` path → the full-chrome binary — then `E2E_LOCAL_SERVER=1` build with CI dummy-Supabase env. The 5 skips are the Supabase-seeded profile-structural tests (expected). lyra-mcp-server (`main` HEAD `7b1a90a`): `tsc` clean + **585/585** (38 suites) PASS. | **None fixed — no product defect, no test modified/skipped/weakened** (Test Integrity upheld). npm audit: lyra prod **0**, dev 3 moderate (dev-only). Both raw-runner UNVERIFIED lines are the known BUGS-51/BUGS-58 env gaps, not code. SEC-35 (qs DoS, mcp-server) already has **3 draft PRs (#91/#95/#97)** + dependabot #59 → did **not** open a 4th. BUGS-51 fix already on develop; stale dup draft **#388** could be closed. | **Report-only (out of autonomous bounds):** open High/Highest are all security/RLS/auth/architectural — **SEC-2, SEC-3, SEC-23/30/31/32/34/38/43, BUGS-56, BUGS-57** (prod MCP OAuth rejected). **Infra ask:** bake matching Playwright browsers into the routine's setup (or the pre-baked-chromium symlink shim) so `e2e` reads VERIFIED not UNVERIFIED weekly (BUGS-51 §5 / BUGS-58). Dependabot backlog stale >7d (BUGS-64). | **NO auto-promote — release HOLDS FEATURES.** `origin/main..origin/develop` = **11 commits**, incl. multiple `feat()`: `feat(dashboard)` remove profile-completion pressure (#418), `feat(profile)` editor tidy-up KAN-404 (#416), `feat(ux)` KAN-349 dashboard widget/postcode→city/gifts de-gate/share (#408). Per boundary 1, features → **manual prod sign-off required**: `gh workflow run promote-to-staging.yml -f confirm=promote` → verify → `promote-staging-to-beta.yml -f confirm=promote` → verify → `promote-to-production.yml -f confirm=PRODUCTION`. No promotion / no push to staging/beta/main this run. |
