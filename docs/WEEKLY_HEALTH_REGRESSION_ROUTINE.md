...
| 2026-07-04 | Weekly Health+Regression (Claude Code, remote) | Health **green** (`checklyra.com/api/health` ok, `vercelEnv=production`; `mcp.checklyra.com/health` ok; develop HEAD `b02fdf9` Deploy-to-Development + CodeQL both green). lyra **PASS=5** (lint / type-check / unit **1900**/154 suites / scripts **69**/7 suites / build); `integration`+`e2e` **UNVERIFIED** env-gaps (BUGS-51), **not** FAIL. lyra-mcp-server: `tsc` clean + **585/585** (38 suites) PASS. | **No product bug — zero FAILs.** No test modified/skipped/weakened (Test Integrity Policy). Both UNVERIFIED are BUGS-51 env gaps: no `tests/integration/` dir → jest `No tests found`; and pre-baked Playwright `chromium-1194`/no-webkit ≠ `@playwright/test@1.56.1`'s required rev **1223** (`chrome-headless-shell-1223` + webkit absent, CDN blocked) — a chromium-only local-build run also can't launch, confirming it's an env gap not a defect. Runner correctly classified both UNVERIFIED (BUGS-51 fix #404 working). | **Release holds features → manual prod sign-off (below).** Open **HIGH** SEC untouched (SEC-3/23/30/31/32/43) + admin-MCP SEC cluster + BUGS-61/62/66 — all security/RLS/auth/architectural/data-model → out of autonomous bounds, report-only. Dep vulns: lyra **prod 0** (3 dev-only moderate: js-yaml + uuid via `@lhci`); mcp-server **prod 1** moderate (`qs` DoS GHSA-q8mj-m7cp-5q26, already tracked SEC-35/41/59 + draft PR). | **NO auto-promote — features pending.** `origin/main..origin/develop` = **15 commits**, incl. features: `feat(dashboard)` remove completion pressure (#418), `feat(profile)` KAN-404 editor (#416), `feat(ux)` KAN-349 (#408). Fix-only gate **not** met → **MANUAL** sign-off required: `promote-to-staging.yml` → verify → `promote-staging-to-beta.yml` → verify → `promote-to-production.yml -f confirm=PRODUCTION`. |
