...
| 2026-07-06 | Weekly Health+Regression (Claude Code, remote) | Health **GREEN**: `checklyra.com/api/health` ok (prod); `mcp.checklyra.com/health` ok; mcp `build_sha` = origin/main HEAD `e544531`; develop HEAD `1bac682` CI green. lyra **PASS=5** (lint/type-check/unit/scripts/build), FAIL=0, `integration`+`e2e` env-gap **UNVERIFIED** (BUGS-51 — no `tests/integration/` dir; chromium rev `1194` ≠ `@playwright/test@1.61.1` required `1228`, CDN blocked). **E2E manually re-verified REAL-GREEN** against a locally-served prod build using the installed chromium via a throwaway `executablePath` override (no checked-in config/test touched): **17 passed / 5 skipped / 0 failed**. lyra-mcp-server (origin/main `e544531`): `tsc` clean + **637/637** (41 suites) PASS. | **No product bug; no test modified/skipped/weakened.** npm audit: lyra prod **0**, mcp prod **1 moderate** (`qs` DoS = **SEC-35**; transitive + non-exploitable/defence-in-depth; fix PR #91 was **owner-closed unmerged 2026-07-04** → left as-is per that decision). | **Release HELD (features).** Also citing (owned by Daily Security, not this routine): **SEC-79** watchdog FAIL — `health-check.yml` + `weekly-report.yml` `disabled_manually` since 2026-06-16. No other STOP items new this run. | **Held — not fix-only.** `origin/main..origin/develop` = **37 commits** incl. features + a migration (KAN-404 profile-editor rework, `feat` dashboard/profile/ux #418/#416/#408, SEC-54/56 SECURITY DEFINER+RLS migration #443) → **manual prod sign-off required**: `gh workflow run promote-to-production.yml -f confirm=PRODUCTION` (after staging→beta). No promotion run (prepared + reported only). |
