...
| 2026-07-05 | Weekly Health+Regression (Claude Code) | **Green — zero product FAILs.** Health: `checklyra.com/api/health` ok (prod); `mcp.checklyra.com/health` ok; develop HEAD `b258a07` CI (Deploy to Development + CodeQL) green. lyra: `RUN_E2E=1 weekly-health-regression.sh` → **PASS=5** (lint/type-check/unit/scripts/build) / **FAIL=0** / **UNVERIFIED=2**. lyra-mcp-server: `tsc` clean + **637/637** (41 suites) PASS. | **None to fix — no application defect.** Both UNVERIFIED lines are the known **BUGS-51** env gaps, not code: (a) `integration` — no `tests/integration/` dir → jest 0 matches; (b) `e2e` — sandbox pre-baked chromium rev `1194` ≠ `@playwright/test@1.61.1`'s required rev `1228` (Playwright CDN download-blocked, 403), so tests skip/can't launch — never an assertion failure. No test modified/skipped/weakened (Test Integrity Policy). | **BUGS-51 persists** (browser lockstep gap widened by the #364 `@playwright/test` 1.60→1.61.1 bump, 1223→1228) — needs infra: bake rev-1228 browsers or allowlist the CDN. Open HIGH SEC items (SEC-78/3/43/32/31/30/23) + all other open BUGS/SEC are security/RLS/architectural/data-model → **report-only, out of autonomous bounds**. npm audit: lyra prod **0**, dev **3 moderate** (all dev-only `@lhci/cli` transitive: js-yaml/uuid); mcp-server **0**. | **NO auto-promote — release holds features.** `origin/main..origin/develop` = **35 commits**, many are features/migrations (KAN-404 profile-editor rework #416/#439/#440/#441/#442, `feat(dashboard)` #418/#408, `feat(profile)` #416, `feat(ux)` #408, SEC-54/56/BUGS-65 SECURITY-DEFINER+RLS migration #443, KAN-348 E2E harness #436). **Manual prod sign-off required** — see ACTION NEEDED. |
