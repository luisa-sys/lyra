...
| 2026-07-03 | Weekly Health+Regression (Claude Code, remote) | Health **green** (`checklyra.com/api/health` ok, `vercelEnv=production`; `mcp.checklyra.com/health` ok; develop HEAD `3800836` CI: Deploy + CodeQL both success). lyra **PASS=5** (lint/type-check/unit/scripts/build), FAIL=0, **UNVERIFIED=2** (`integration` — no `tests/integration/` dir; `e2e` — env gap). lyra-mcp-server: `tsc` clean + **585/585** (38 suites) PASS. | **No product bug — nothing to fix; no test modified/skipped/weakened.** Both UNVERIFIED lines are the known **BUGS-51** env gap: pre-baked Playwright browsers rev `1194` ≠ `@playwright/test@1.60.0`'s required `1223` (`Executable doesn't exist at …chromium_headless_shell-1223`, never an assertion) and the Playwright CDN is network-blocked (`npx playwright install` → Download failure). npm audit: **prod 0 vulns both repos**; dev-only moderates (lyra: js-yaml/uuid ×3; mcp: qs/other ×3, SEC-35 tracked). | **STOP — release holds features (manual prod sign-off).** No autonomous fixes were required. Open HIGH BUGS/SEC (BUGS-56/57, SEC-2/3/23/30/31/32/34/38/43) all untouched — security/RLS/auth/architectural, out of autonomous bounds. E2E remains unverifiable in-sandbox until infra bakes rev-1223 browsers or allowlists the Playwright CDN (BUGS-51 §5). | **NO auto-promote — release holds features.** `origin/main..origin/develop` = **11 commits**, of which **3 are `feat()`**: #418 (dashboard completion-pressure removal), #416 (KAN-404 profile-editor tidy-up), #408 (KAN-349 dashboard widget journey/postcode→city/gifts de-gate/share). Not fix-only → **manual sign-off required**: `gh workflow run promote-to-staging.yml -f confirm=promote` → verify → `promote-staging-to-beta.yml -f confirm=promote` → verify → `promote-to-production.yml -f confirm=PRODUCTION`. |
