...
| 2026-07-01 | Weekly Health+Regression (Claude Code, remote) | Health **GREEN** (`checklyra.com/api/health` ok/prod; `mcp.checklyra.com/health` ok; develop HEAD `c10d3dd` Deploy-to-Dev + CodeQL green). lyra **PASS=5** (lint/type-check/unit/scripts/build); `integration`+`e2e` **UNVERIFIED** env gaps (no `tests/integration/` dir → 0 matches; Playwright browsers absent → 10 skipped) — BUGS-51/BUGS-58, not product FAILs. lyra-mcp-server: `tsc` clean + **585/585** (38 suites) PASS. | **None** — FAIL=0, no product regression found; no test modified/skipped/weakened (Test Integrity Policy). | **qs DoS (SEC-35, MEDIUM)** still unmerged on mcp-server with **4 competing draft PRs** (#97→6.15.3, #95 override, #91→6.15.2, Dependabot #59→6.15.2) — recommend landing **#97** (fully-patched 6.15.3) and closing the rest. Underlying risk low (transitive-only, path unreachable per SEC-35). All other open High/Med BUGS/SEC are security/RLS/auth/architectural → report-only. Prior weekly rows (06-24…06-30) remain in unmerged draft run-log PRs — needs consolidation. | **HELD — feature pending → manual sign-off.** `origin/main..origin/develop` = 8 commits incl. `feat(ux): KAN-349` (#408: dashboard widget journey, postcode→city, gifts de-gate, share) → NOT fix-only, auto-promote MUST NOT fire. Manual chain: `promote-to-staging.yml -f confirm=promote` → verify → `promote-staging-to-beta.yml -f confirm=promote` → verify → `promote-to-production.yml -f confirm=PRODUCTION`. |
