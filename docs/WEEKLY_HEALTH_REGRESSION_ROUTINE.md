...
| 2026-06-26 | Claude Code (weekly routine) | **lyra:** lint/type-check/unit/scripts/build **PASS** (5/5). integration **UNVERIFIED** (no `tests/integration` suite exists — not in floor). e2e **UNVERIFIED** (pinned PW browser build 1223 not installable in sandbox — 403; only 1194 chromium pre-installed, no webkit; no `.env.local`). Chromium-only via pre-installed binary: 4/6 public-pages pass; the 2 misses are solely missing `NEXT_PUBLIC_SUPABASE_URL` → 500, **not** code defects. **lyra-mcp-server:** tsc clean, **583/38 PASS**. Health: checklyra.com + mcp(.dev).checklyra.com all `ok`; develop CI green at HEAD. **No real regression.** | qs DoS GHSA-q8mj-m7cp-5q26 → lyra-mcp-server#95 (SEC-41, draft, awaits human merge to prod-`main`). Runner false-FAIL classification → this PR (BUGS-58, draft, awaits review). | (1) **prod promote holds FEATURES** (Convene, admin redesign, RS256, Didit age-verif, …) → **manual sign-off required**. (2) 18 dev-only jest→js-yaml moderate advisories → need jest major bump (SEC-41). (3) Pre-existing local `claude/nice-meitner-2h2ro8` was contaminated with stray prod-promote commits; reset to clean `origin/develop` before work (BUGS-17 recovery). | **NOT auto-releasable** — features pending; manual prod gate |
