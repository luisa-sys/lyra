...
| 2026-06-21 | Claude Code (1st live run) | lyra: PASS lint/type-check/unit/scripts/build; integration + e2e UNVERIFIED (env gaps — see below), 0 real FAIL. mcp-server: 534 tests / 33 suites PASS. Prod health OK (web + MCP); develop CI + CodeQL green; mcp `build_sha` = main HEAD. | No product defects. Filed **BUGS-51** (runner false-positive: env gaps reported as FAIL not UNVERIFIED) → fixed in this PR (script honesty fix + 4 net-new tests). | (1) **Sandbox can't run E2E**: baked Playwright browsers rev 1194 ≠ `@playwright/test@1.60.0`'s required rev 1223 (+webkit 2287), CDN blocked → browsers won't launch. Authoritative `e2e-tests.yml` PR-gate ran full E2E on develop HEAD → green. (2) No `tests/integration/` exists → `test:integration` finds 0 tests. (3) **Release holds a FEATURE** (#350 SEC-4 `/status`) → prod promote needs manual sign-off. | Develop green but **holds a feature → NOT fix-only → manual prod sign-off required** (`gh workflow run promote-to-production.yml -f confirm=PRODUCTION`, after staging→beta). No promotion performed. |
