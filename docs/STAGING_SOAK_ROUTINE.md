...
|---|---|---|---|---|---|
| 2026-07-25 18:29 | staging-soak routine (KAN-413), run 2 | PASS=12 / FAIL=0 / UNVERIFIED=1 (C3) | pages p50 299–501ms max 604ms / health p50 289ms max 363ms | none | C1/C2/C4 all PASS via `e2e-stage` alias (C1 gate on real host: 302, gated; `/api/health` conformant). **C3 UNVERIFIED — harness/config, two blockers, not a staging bug**: (1) routine env cannot fetch Playwright chromium v1228 (pinned `@playwright/test` 1.61.1) — the browser CDN is unreachable from the container (curl 000; network-allowlist gap; pre-installed build is 1194, mismatched). Fix: allowlist the Playwright download hosts (`cdn.playwright.dev`, `playwright.download.prss.microsoft.com`) or pre-provision the matching build. (2) GoTrue admin listUsers still 500 (verified by direct curl): run-1's seed-row normalisation landed **partially** — `confirmation_token`/`recovery_token` are now `''` on all 8 seed rows, but `email_change` is still NULL on those same 8 rows (8 of 9 users), which still breaks the GoTrue user scan; extend the normalisation to `email_change=''`. Soak user `soak@seed.checklyra.com` still not created. Also noted: `RESEND_API_KEY` not set in the routine env — a hard-FAIL run could not send the alert email (test-plan A4 gap). Updated on KAN-413. C5 at baseline with one benign delta: advisors 10 security / 154 performance (no new); NULL conf/rec-token rows now **0** (was 8 baseline — partial normalisation applied since run 1; 0 non-seed); invite queue 0 stuck >24h; 0 stale one_time_tokens; migration parity OK — DB head `20260724231235` = `bugs69_revoke_global_feature_switch_trigger_exec` matches origin/staging head file `20260725000500_bugs69_…` (accepted apply-timestamp offset). C6 clean: only 24h runtime-error group is the prod-attributed BUGS-71 signature (`get_metrics_for_window` permission denied, `/admin/monitoring`, last 14:14 UTC); liveness cited from health-check.yml: success 2026-07-25 13:45 UTC. No email (no hard FAIL). |
