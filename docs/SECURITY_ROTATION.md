...
| SUPABASE_DB_URL (pg_dump) | GitHub secret | Annual | Supabase → Settings → Database → Connection string. **⚠️ Two consumers, two requirements:** the weekly public-only `backup-database.yml` is fine with the Transaction Pooler (port 6543); the SEC-23 `backup-complete.yml` REQUIRES the **Session / Direct connection (port 5432)** because `pg_dumpall --roles-only` cannot run over the 6543 pooler. Set the **5432 session** string so both work — the complete backup's integrity gate now fails loud if roles were skipped (which a 6543 URL causes). Password changes if the database password is reset. | Initial setup |
...
| NPM_TOKEN (`@lyra/contracts` registry read) | Vercel env vars + Railway env vars on **3 services** (lyra-mcp-server dev, lyra-mcp-server prod, lyra-admin-mcp-server) | 90 days | GitHub → Settings → Developer Settings → Personal access tokens → **Tokens (classic)** → Generate new. Scope: **`read:packages`** only. Update the Vercel env var and **all three** Railway services, then redeploy each. **⚠️ HARD EXPIRY 2026-10-28** — this is a classic PAT with a fixed expiry date, not a rotate-when-convenient secret. **The failure mode is why this row exists:** the token is *inert until `@lyra/contracts@0.1.0` publishes*, so nothing fails today and an unnoticed expiry surfaces later as **four unrelated-looking build failures** across Vercel and three Railway services, with no error naming the token. Rotate before 28 Oct 2026 regardless of publish state. KAN-418. | 29 July 2026 |
