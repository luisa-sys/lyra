...
| 2026-07-06 | cloud-routine (KAN-294/296) | 0 | 0 (+1 new, ops-resilience) | **SEC-79** (🟠 `health-check.yml` + `weekly-report.yml` found `disabled_manually` since 2026-06-16 — liveness/reporting backstop silently off 20 days) | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED (site+MCP fully reachable). Supabase B1–B9 all three envs (prod/staging/dev): 0 RLS-disabled ✅, vault RPCs anon/auth=false ✅ (SEC-01 holds), 0 definer views + `mcp_per_ip_recent_count` invoker-mode ✅ (SEC-02 holds), both privesc-guard triggers present+enabled (`profiles_block_admin_is_suspended_self_set`, `profiles_prevent_beta_self_elevation` — note actual names are `profiles_`-prefixed, not the bare names in this doc's literal query), 0 SQLi-shaped definer fns ✅. B9 residual grants (`admin_filter_profile_ids`/`admin_list_users` auth-exec, `block_admin_is_suspended_self_set`/`prevent_feature_entitlement_self_grant` still PUBLIC-exec on prod+staging) all map to open/in-progress tickets — **SEC-43**, **BUGS-65** — confirmed via `pg_get_functiondef` (admin RPCs self-gate on `is_admin`, BUGS-60) and trigger-return-type exemption (not RPC-invokable); none new. `profile-photos` bucket still `public=true` — known **SEC-60**, not new. B8 migration parity: dev carries **SEC-45** (High, profile_items visibility RLS leak) and **SEC-54/SEC-56/BUGS-65** fixes not yet promoted to staging/prod — both already open/in-progress (not new tickets), but flagging: SEC-45 is High severity and its 48h fix-SLA (opened 2026-07-04 08:06) is about to lapse without a staging/prod promotion. GitHub: CODEOWNERS + SECURITY.md present on `lyra`; `lyra-mcp-server` `main` HEAD (`e544531`) matches the MCP `/.well-known/mcp.json` `build_sha` ✅ (C7 holds). Branch-protection / Dependabot / code-scanning / secret-scanning alert counts again **UNVERIFIED** — no direct API access from this session, same standing gap. Cloudflare: 1 KV ns, 1 R2 bucket, 1 Worker — matches expected inventory (A8 clean, DP-04 tracked). **New this run:** built the first Ops-Routines-Control-Room heartbeat (KAN-362) and fed real GitHub Actions state into `scripts/routine-watchdog.sh` — discovered `health-check.yml` (liveness-of-record) and `weekly-report.yml` both `disabled_manually` since 2026-06-16 (8-workflow mass-disable, no ticket/commit explaining it), contradicting KAN-361's just-merged design (PR #437, 2026-07-04) which assumes both are live. Filed **SEC-79**. Watchdog result: PASS=3, FAIL=2, UNVERIFIED=1 (doc-producer, no heartbeat on record yet), exit 2. Per the Control Room's alerting rule this should also trigger `scripts/security-alert-email.sh` — **deliberately not run this session**: the task dispatch for this run scoped mutations to Jira-ticket-creation + a docs-only draft PR only, so the email step is deferred to the operator (see PR description) rather than silently skipped. |
