...
| 2026-07-02 | cloud-routine (KAN-294/296) | 0 | 0 | none — every flagged item already tracked (no new BUGS/SEC filed) | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED (site+MCP fully reachable). B2 0 unprotected tables all 3 envs ✅. B3 vault RPCs anon/auth=false all envs ✅ SEC-01 holds. B4 `mcp_per_ip_recent_count` `security_invoker=on`, 0 definer views ✅ SEC-02 holds. B5 `profile-files` private + owner-scoped policies ✅ SEC-03 holds; `profile-photos` `public=true` residual unchanged (tracked **SEC-60**, open). B6 both privesc triggers (`profiles_prevent_beta_self_elevation`, `profiles_block_admin_is_suspended_self_set`) present + enabled ('O') all 3 envs; confirmed `prevent_beta_self_elevation`'s guard was extended to cover the new access-model columns (`user_status`/`access_tier`/`age_status`), not just legacy ones — no privesc gap from the KAN-326 migration. B7 0 SQLi-shaped dynamic-SQL fns all envs ✅. B8: prod is missing the `kan_143_profile_items_visibility`/`kan_153_phone_postcode_search` migrations present on staging/dev — matches the in-flight **KAN-339** postcode walk-back/**KAN-341** replacement, not unplanned drift. B9: 4 anon/auth-executable definer fns on all envs — 2 are `trigger`-return (inert via PostgREST, tracked **BUGS-65** Low) + `admin_list_users`/`admin_filter_profile_ids` (internally `auth.uid()`+`is_admin` guarded, not exploitable; regression already tracked **SEC-42** HIGH / **SEC-43**, both still open — reconfirmed present, no new filing). GitHub: CODEOWNERS + SECURITY.md present; branch-protection/dependabot/secret-scanning/code-scanning alert counts UNVERIFIED — no exposed branch-protection/alerts-listing tool in this session's GitHub MCP surface (tracked **BUGS-64**, **SEC-77**, **SEC-66**). C7: MCP `build_sha` matches `origin/main` HEAD exactly (`7b1a90a4…`) — no deploy drift. Cloudflare: KV=1 (`lyra-interest-emails`, DP-04 tracked), R2=1 (`lyra-opennext-cache`), Workers=1 (`lyra-maintenance`) — no new/unexpected resources. |
