...
| 2026-07-03 | cloud-routine (KAN-294/296) | 0 | 0 | None — all findings already covered by open tickets | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED (full site+MCP reachability ✅). C7 build_sha `7b1a90…` matches `origin/main` HEAD exactly ✅. B1–B9 run on all three Supabase projects: B2 0 RLS-disabled tables ✅; B3 vault RPCs anon/auth=false ✅ (SEC-01 holds); B4 `mcp_per_ip_recent_count` still `security_invoker=on` ✅ (SEC-02 holds); B5 `profile-files` private ✅, `profile-photos` still `public=true` (tracked SEC-60, open, not new); B6 **both** `profiles_block_admin_is_suspended_self_set` and `profiles_prevent_beta_self_elevation` triggers present+enabled on prod/staging/dev ✅ (SEC-27 fix holds, no privesc regression); B7 0 SQLi-pattern definer fns ✅; B8 dev carries 2 extra non-security migrations ahead of prod (tracked SEC-8/OPS-04, open, not new); B9 prod still shows `admin_filter_profile_ids`/`admin_list_users` authenticated-exec (internally admin-guarded, confirmed via `pg_get_functiondef` — not exploitable) plus 2 trigger-type fns — **already tracked by SEC-43 (open) and BUGS-65 (open)**, no new ticket filed. Security-advisor WARN count for `auth_rls_initplan` drifted 33→37 (tracked SEC-11/BUGS-27, open, not new). Cloudflare KV/R2/Workers inventory unchanged (1/1/1, no new assets). CODEOWNERS + SECURITY.md present in both repos. GitHub alert counts (code-scanning/dependabot/secret-scanning) + branch protection = UNVERIFIED — GitHub MCP connector still has no alert-list/get-branch-protection tool (longstanding, documented gap — not new). No new 🔴/🟠; zero tickets filed this run. |
