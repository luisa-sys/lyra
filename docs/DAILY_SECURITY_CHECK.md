...
| 2026-07-01 | cloud-routine (KAN-294/296) | 0 | 0 | **BUGS-64** (🟠 dependency-PR backlog stale, GOV-03 — first ticket for this row), **BUGS-65** (🔵 residual PUBLIC-execute on 2 new trigger fns, B9) | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED — site+MCP fully reachable. B2 (RLS) 0 rows all envs ✅. B3 vault RPCs anon=false/auth=false all 3 envs ✅ SEC-01 holds. B4 `mcp_per_ip_recent_count` security_invoker=on ✅ SEC-02 holds. B5 profile-photos `public=true` residual unchanged (tracked, listing owner-scoped) ✅ SEC-03 holds on prod. B6 both privesc triggers present+enabled on prod (`profiles_block_admin_is_suspended_self_set`, `profiles_prevent_beta_self_elevation`) ✅ — narrower B6 SQL initially missed the beta-elevation trigger due to naming, confirmed present via broader query, not a regression. B7 (SQLi dynamic-concat) 0 rows ✅. B9: prod shows 4 anon/auth-executable definer fns vs "0" at 2026-06-22 triage, but verified safe — `admin_filter_profile_ids`/`admin_list_users` carry internal `is_admin` guards (authenticated-exec is intentional per BUGS-60), `block_admin_is_suspended_self_set`/`prevent_feature_entitlement_self_grant` are trigger-return (not RPC-invokable) but still PUBLIC-executable → **BUGS-65 filed, Low** (same class as closed BUGS-48, functions postdate that fix). F-04 `search_by_contact_hash`: absent on prod (feature not yet promoted — `kan_153_phone_postcode_search` migration missing from prod, known OPS-04 drift); staging/dev show anon=false/auth=true, matching the BUGS-45/47 fixed state — no new finding. Jira cross-check found several former BUGS security tickets (BUGS-24→SEC-12, BUGS-25→SEC-13, BUGS-26→SEC-9, BUGS-27→SEC-11, BUGS-44→SEC-15, BUGS-52→SEC-27, BUGS-53→SEC-28) have since been moved into the **SEC** project, all **Done** — housekeeping note, no action needed. E1 GOV-01 (SEC-3) still **In Progress** (unchanged, no regression). E3: dependency-PR sweep across both repos found 15 of 17 open `dependencies`-labelled PRs stale >7 days (vs. 1 stale PR at 2026-06-22) — same `jose` 5.10.0→6.2.3 bump open 44 days in **both** repos untouched → **BUGS-64 filed, Med** (GOV-03's first ticket). E4 `pr-checks.yml` (lyra) and `test.yml` (lyra-mcp-server) latest runs both `success`. C7 MCP `build_sha` (`7b1a90a4f6e7a1e221ee0dac5cb986c75f2a45fb`) matches `origin/main` HEAD exactly ✅. Cloudflare: KV (`lyra-interest-emails`), R2 (`lyra-opennext-cache`), Workers (`lyra-maintenance`) inventory unchanged from baseline — DP-04 still open/tracked, no new store. G1 OAuth metadata unchanged (PKCE S256, expected endpoints). Performance advisors (prod): 0 ERROR, 68 WARN (`auth_rls_initplan` grew 33→37 — tracked under SEC-05, not filed separately). Branch protection / code-scanning / dependabot-alert-count / secret-scanning-alert-count (E1 detail, E2, D1) remain **UNVERIFIED** — no GitHub MCP tool exposes these endpoints and no `gh` CLI access this session; re-run from an allowlisted host with `gh`/API creds per the doc's own caveat. |
