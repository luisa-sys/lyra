...
| 2026-07-01 | cloud-routine (KAN-294/296) | 0 | 0 | **BUGS-64** (🟠 dependency-PR backlog stale, GOV-03 — first ticket for this row), **BUGS-65** (🔵 residual PUBLIC-execute on 2 new trigger fns, B9) | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED — site+MCP fully reachable. B2 (RLS) 0 rows all envs ✅. B3 vault RPCs anon=false/auth=false all 3 envs ✅ SEC-01 holds. B4 `mcp_per_ip_recent_count` security_invoker=on ✅ SEC-02 holds. B5 profile-photos `public=true` residual unchanged (tracked, listing owner-scoped) ✅ SEC-03 holds on prod. B6 both privesc triggers present+enabled on prod (`profiles_block_admin_is_suspended_self_set`, `profiles_prevent_beta_self_elevation`) ✅. B7 (SQLi dynamic-concat) 0 rows ✅. B9: prod shows 4 anon/auth-executable definer fns vs "0" at 2026-06-22 triage, but verified safe — `admin_filter_profile_ids`/`admin_list_users` carry internal `is_admin` guards (authenticated-exec intentional, admin dashboard needs it), `block_admin_is_suspended_self_set`/`prevent_feature_entitlement_self_grant` are trigger-return (not RPC-invokable) but still PUBLIC-executable → **BUGS-65 filed, Low**. Jira cross-check found several former BUGS security tickets (BUGS-24→SEC-12, BUGS-25→SEC-13, BUGS-26→SEC-9, BUGS-27→SEC-11, BUGS-44→SEC-15, BUGS-52→SEC-27, BUGS-53→SEC-28) have since been moved into the **SEC** project, all **Done** — housekeeping note. E3: dependency-PR sweep across both repos found 15 of 17 open `dependencies`-labelled PRs stale >7 days — `jose` 5.10.0→6.2.3 bump open 44 days in **both** repos untouched → **BUGS-64 filed, Med** (GOV-03's first ticket). C7 MCP `build_sha` matches `origin/main` HEAD exactly ✅. Cloudflare inventory unchanged. Branch protection / code-scanning / dependabot-alert-count / secret-scanning-alert-count remain **UNVERIFIED** — no GitHub MCP tool exposes these endpoints this session. |
| 2026-07-02 | cloud-routine (KAN-294/296) | 0 | 0 | none — every flagged item already tracked (no new BUGS/SEC filed) | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED. B2–B7 clean/holding as prior days. B6 confirmed `prevent_beta_self_elevation`'s guard was extended to cover the new access-model columns (`user_status`/`access_tier`/`age_status`), not just legacy ones — no privesc gap from the KAN-326 migration. B8: prod missing `kan_143_profile_items_visibility`/`kan_153_phone_postcode_search` present on staging/dev — matches in-flight KAN-339 postcode walk-back/KAN-341 replacement, not unplanned drift. B9: 4 anon/auth-executable definer fns on all envs — 2 trigger-return (BUGS-65, Low) + `admin_list_users`/`admin_filter_profile_ids` (admin-guarded, tracked **SEC-42**/**SEC-43**, both open — reconfirmed, no new filing). GitHub branch-protection/dependabot/secret-scanning/code-scanning alert counts UNVERIFIED (tracked **BUGS-64**, **SEC-77**, **SEC-66**). C7 build_sha matches origin/main HEAD. Cloudflare inventory unchanged. |
| 2026-07-03 | cloud-routine (KAN-294/296) | 0 | 0 | None — all findings already covered by open tickets | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED. C7 build_sha matches origin/main HEAD ✅. B2–B7 clean/holding all envs. B8 dev carries 2 extra non-security migrations ahead of prod (SEC-8/OPS-04, open, not new). B9 prod still shows `admin_filter_profile_ids`/`admin_list_users` authenticated-exec (internally admin-guarded, confirmed via `pg_get_functiondef` — not exploitable) plus 2 trigger-type fns — already tracked by **SEC-43** (open) and **BUGS-65** (open), no new ticket filed. `auth_rls_initplan` WARN count drifted 33→37 (SEC-11/BUGS-27, open, not new). Cloudflare inventory unchanged. CODEOWNERS + SECURITY.md present both repos. GitHub alert counts + branch protection UNVERIFIED (longstanding, documented gap). |
| 2026-07-04 | cloud-routine (KAN-294/296) | 0 | 0 | None — every 🟠/🟡 already covered by an open ticket; no new finding this run | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED (site+MCP fully reachable, full egress this run ✅). **B1** advisors (type=security) on all 3 Supabase projects: 0 ERROR; WARNs are the same known set (5× `rls_enabled_no_policy` INFO on service-role-only tables; `block_admin_is_suspended_self_set`+`prevent_feature_entitlement_self_grant` anon/auth-exec trigger fns → **BUGS-65** open; `admin_filter_profile_ids`+`admin_list_users` authenticated-exec → **SEC-43** open). **B2** 0 RLS-disabled tables, all envs ✅. **B3** vault RPCs `anon=false,auth=false` all envs ✅ SEC-01 holds. **B4** 0 definer views; `mcp_per_ip_recent_count` `security_invoker=on` all envs ✅ SEC-02 holds. **B5** `profile-files` private + owner-scoped policies (`(storage.foldername(name))[1]=auth.uid()::text`) all envs ✅; `profile-photos` `public=true` residual unchanged → **SEC-60** open, not new. **B6** — re-verified directly via `pg_trigger` (not just the doc's probe SQL, which still references the pre-KAN-326 trigger name): both `profiles_block_admin_is_suspended_self_set` and `profiles_prevent_beta_self_elevation` present and `tgenabled='O'` on **all 3 envs** ✅ — SEC-27 fix holds, no privesc regression. **B7** 0 dynamic-SQL-concat SECURITY DEFINER fns, all envs ✅. **B9** prod/staging/dev: same 4-function set as recent runs — `admin_filter_profile_ids` (`uuid[]`, auth_exec=true) + `admin_list_users` (`json`, auth_exec=true) confirmed still open under **SEC-43** (verified via Jira: status To Do); `block_admin_is_suspended_self_set`+`prevent_feature_entitlement_self_grant` (trigger-return, PUBLIC-exec) confirmed under **BUGS-65** (To Do). Staging/dev additionally show `search_by_contact_hash` `auth_exec=true,anon_exec=false` (matches the fixed/intended state, BUGS-45/47); absent entirely on prod (feature migration not yet promoted — known OPS-04/KAN-339 drift, not new). **B8** migration list confirms the same prod-vs-staging/dev gap as 07-02/07-03 (postcode-search/profile-items-visibility feature migrations only — no security migration missing from prod). **Cloudflare A8**: KV=1 (`lyra-interest-emails`, DP-04 tracked open), R2=1 (`lyra-opennext-cache`), Workers=1 (`lyra-maintenance`) — inventory unchanged, no new/unexpected resources. **GitHub**: CODEOWNERS ✅ + SECURITY.md ✅ present (`luisa-sys/lyra`). E3 dependency-PR sweep: `luisa-sys/lyra` 10/10 open `dependencies`-PRs still >7 days stale (oldest `#278` jose 5→6, now 47 days); `luisa-sys/lyra-mcp-server` 5/7 stale (`#53` jose 5→6 also 47 days — same unmerged major bump in both repos); 2 fresh (`#98`/`#99`, 5 days) — continuing, already tracked **BUGS-64** (To Do), not re-filed. **C7**: `lyra-mcp-server` `main` HEAD `7b1a90a4f6e7a1e221ee0dac5cb986c75f2a45fb` matches `mcp.checklyra.com`'s advertised `build_sha` exactly ✅ — no deploy drift. Branch-protection / code-scanning / dependabot-alert-count / secret-scanning-alert-count (E1 detail, E2, D1) remain **UNVERIFIED** — no GitHub MCP tool exposes these endpoints this session (tracked under **BUGS-64**/**SEC-77**/**SEC-66**, all To Do, not new). **Housekeeping note:** the 2026-07-01/02/03 run-log rows existed only on unmerged `claude/*` branches (stale draft PRs) — folded into this row's PR so `develop`'s copy of this doc catches up; those tickets (SEC-42/43/60/64/65/66/77) were independently confirmed live in Jira before being cited here, not assumed from the unmerged commit text. No new SEC/BUGS ticket filed this run — every 🟠/🟡 observed maps to an already-open ticket. |
