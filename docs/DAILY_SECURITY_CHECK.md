...
| 2026-07-05 | cloud-routine (KAN-294/296) | 0 | 0 | none — every gap found already covered by an open/in-progress ticket | Script: 21 PASS / 0 FAIL / 0 UNVERIFIED (site+MCP fully reachable this run). B2 0 RLS-disabled ✅ all envs. B3 vault RPCs anon/auth=false ✅ all envs (SEC-01 holds). B4 0 definer views, `mcp_per_ip_recent_count` security_invoker=on ✅ (SEC-02 holds). B5: `profile-files` private + owner-scoped policies ✅; `profile-photos` still `public=true` — known-open **SEC-60**, not new. B6: both privesc-guard triggers present+enabled all envs (`profiles_block_admin_is_suspended_self_set`, `profiles_prevent_beta_self_elevation` — note the trigger is prefixed `profiles_`, not a bare name; confirmed via `pg_trigger` on `public.profiles` after the regression-map's literal name list returned 0 rows). B7 0 SQLi-shaped definer fns ✅. B9: `admin_filter_profile_ids`/`admin_list_users` still `auth_exec=true` all envs — tracked **SEC-43** (open); `block_admin_is_suspended_self_set`/`prevent_feature_entitlement_self_grant` still PUBLIC-exec on prod (both `RETURNS trigger`, not RPC-invokable) — tracked **BUGS-65** (in progress); `search_by_contact_hash` auth-exec is intentional (BUGS-47, done). B1 advisors: WARN-only on all 3 envs, same set as above, 0 ERROR, count did not increase. B8 migration parity: dev has a handful of not-yet-promoted migrations (dashboard widget state, postcode scrub, SEC-45/BUGS-65 fixes) — normal promotion lag, OPS-04. GitHub: CODEOWNERS + SECURITY.md present both repos; only collaborator on `lyra` is `luisa-sys` (admin); CodeQL green both repos (latest run 2026-07-04); PR Checks green on live branches (one failure on a stale dependabot eslint-bump branch, consistent with already-tracked **BUGS-64**); `lyra-mcp-server` post-merge-deploy-smoke green (build_sha matches origin/main HEAD, self-healed a transient failure within 10 min on 2026-07-04). Noticed a `.pat-verify-probe.yml` run from 2026-07-01 — investigated: a self-cleaning PAT workflow-scope probe (file already deleted from repo, run `conclusion=failure` = PAT correctly lacks `workflows:write`, least privilege holds); not a finding. Code-scanning/dependabot/secret-scanning alert counts and branch-protection queried directly via GitHub REST API with the session token — all still return "Resource not accessible by integration" / proxy-blocked → **UNVERIFIED**, same standing gap as every prior run (SEC-04/OPS-05/GOV-01 caveat, no admin-scoped token available to this session). Cloudflare: 1 KV ns (`lyra-interest-emails`, DP-04 tracked), 1 R2 bucket (`lyra-opennext-cache`, expected), 1 Worker (`lyra-maintenance`, expected) — no unexpected resources, A8 clean. Backups: DB/platform/restore-drill all last green 2026-06-28 (within 7-day window; today's Sunday run not yet due at check time). `check-secret-rotation.py` exits 0 (PASS) but flags 6 secrets still at "Initial setup" — standing/informational, not new. |
